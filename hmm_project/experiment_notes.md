...
The `memoryless_property_test.png` is present in both archive folders as a
reference (the memoryless analysis is independent of `GEOM_P` — it probes the
natural non-Geometric system with fixed-duration disruption, so the *numbers*
are the same in both folders). The figure was later restyled for visual
consistency with the rest of the project (see Section 6 below) but the
underlying numerical results — mean Disruption dwell 20.0 exact, Recovery
dwell 6.5, LR stat 175.70 (df = 12, $p < 0.001$) — are unchanged.
...
### Memoryless property figure restyle

Same spirit as the confusion-matrix restyle — purely cosmetic, no
underlying computation changed:

- Transition-matrix heatmap (bottom-right panel of the 2×2 figure):
  `cmap='YlOrRd'` → **`cmap='Blues'`**, matching the restyled confusion
  matrix and the Transition Matrix $A$ in `trained_matrices.png`.
- Dwell-time histograms (top two panels): hard-coded `'gold'` for Recovery
  and `'red'` for Disruption were replaced with the canonical
  `STATE_COLORS` palette (Steady green `#2ecc71`, Disruption red
  `#e74c3c`, Recovery orange `#f39c12`) that `visualize.py` uses
  throughout the rest of the project.
- Example state-sequence strip (bottom-left panel): ad-hoc `green / red / gold`
  → same canonical palette, with alpha bumped from 0.4 → 0.6 for better
  contrast against the white background.

The pure-yellow `gold` is gone everywhere, and the figure now reads as
part of the same visual family as `hero_figure.png`,
`forward_probabilities.png`, `raw_signals.png`, and the Blues heatmaps.

Regenerated by rerunning `python test_memoryless.py` (deterministic seeds,
so the 200-run fixed-duration simulation produces identical dwell times to
the pre-restyle version). Verified against `report.md` Section 3.2:

- Disruption dwell: mean **20.0**, std 0.0 (exact by construction) ✓
- Recovery dwell: mean **6.5**, std 2.2 ✓
- First-order vs second-order LR statistic: **175.70**, df = 12, $p < 0.001$ ✓

Output files overwritten in place:
- `hmm_project/results/memoryless_property_test.png` (live pipeline output)
- `hmm_project/p08/results/memoryless_property_test.png` (baseline archive)
- `hmm_project/p04/results/memoryless_property_test.png` (robustness archive)

`report.md` Figure 1 now renders with the new palette without any path
changes.
...
- `5fc7aa4` — memoryless property figure Blues restyle + wrap-up notes
- `<this commit>` — document memoryless restyle in experiment_notes.md
